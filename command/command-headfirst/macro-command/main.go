...
func main() {
	...
	livingRoomOnCommands := []command.ICommand{livingRoomLightOn, ceilingFanOn, stereoOnWithCD}
	livingRoomOffCommands := []command.ICommand{livingRoomLightOff, ceilingFanOff, stereoOff}
	livingRoomMacroOnCommand := command.NewMacroCommand(livingRoomOnCommands)
	livingRoomMacroOffCommand := command.NewMacroCommand(livingRoomOffCommands)

	// then assign MacroCommand to a slot
